...
public static class Logger
{
    ...
    public static void Except(Exception e, LoggingContext? context = null) => throw new NotSupportedException(Format("ERROR", e.ToString(), context));
    private static void Log(string tag,TextWriter output, string message, LoggingContext? loggingContext)
    {
        output.WriteLine(Format(tag,message,loggingContext));
    }

    private static string Format(string tag, string message, LoggingContext? loggingContext)
    {
        DateTime currentTime = DateTime.Now;
        return "[" + tag + "/" + currentTime.ToString("HH:mm:ss") + "] " +
               (loggingContext != null ? loggingContext.ToString() : "") + message;
    }
}